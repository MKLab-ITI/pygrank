...
class ConvergingGraphFilter(ClosedFormGraphFilter):
    """Implements a graph filter with a specific vector of weight parameters."""
    ...
    def __init__(self, weights=None, **kwargs):
        ...
        Example:
            ...
            >>> algorithm = GenericGraphFilter([0.5, 0.25, 0.125], tol=1.E-9) # tol passed to ConvergenceManager
        """
        super(ConvergingGraphFilter, self).__init__(**kwargs)
        self.weights = weights if weights is not None else [0.5]*10

    def _coefficient(self, _):
        if self.convergence.iteration > len(self.weights):
            return 0
        w = 1
        for i in range(self.convergence.iteration):
            w *= self.weights[self.convergence.iteration-1]
        return w