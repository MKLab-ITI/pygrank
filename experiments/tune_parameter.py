...
known, evaluation = split_groups(list(group), training_samples=0.2)
training, validation = split_groups(known, training_samples=0.5)
...
params = optimize(lambda params: -AUC(evaluation, exclude=known).evaluate(GenericGraphFilter([params[0]]*5+[params[1]]*5, to_scipy=pre, max_iters=10000).rank(G, training)),
                  max_vals=[0.99, 0.99], min_vals=[0.1, 0.1], tol=0.01, verbose=True, divide_range="shrinking", partitions=5)
